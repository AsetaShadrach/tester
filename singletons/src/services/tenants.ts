...
export class TenantService {
  ...
  async filterTestAccounts(filterParams:any):Promise<{items:Tenant[],count:number}>{
    let tenants:any;
    
    if(filterParams.params && filterParams.params.id){
      // This section handles UUID types specifically i.e not VARCHAR
      tenants = await this.tenantRepository.findAndCountBy({id:filterParams.params.id})
    }else{
